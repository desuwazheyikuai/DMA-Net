...
from ffcm import FFCM
from pgfn import TextImageFusion
...
class CDModel(nn.Module):
    def __init__(
        ...
        self.ffcm = FFCM(dim=768)
        self.feature_fusion = TextImageFusion(dim=768)
    ...
    def forward(self, input_v, input_q, mask=None, epoch=0):
        ...
        visionFeatures = self.router(m0, m1, m2, v_out)
        fused_feature = self.feature_fusion(visionFeatures, q)

        x = nn.SiLU()(fused_feature)
        x = self.dropout(x)
